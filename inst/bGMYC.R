...
# ── 9.1. СТАНДАРТНАЯ ВИЗУАЛИЗАЦИЯ (plot.interactive.probmat) ──
cat("\n📊 9.1. Создание стандартной интерактивной визуализации...\n")
...
html_path_1 <- file.path(DELIM_DIR, "bGMYC_interactive_standard.html")
...
plot.interactive.probmat(
  ...
  save_html = html_path_1
)

cat(sprintf("✅ Стандартная визуализация сохранена: %s\n", normalizePath(html_path_1)))


#  9.2. КАСТОМНАЯ ВИЗУАЛИЗАЦИЯ (дерево + матрица) 

cat("\n🎨 9.2. Создание кастомной визуализации с деревом (1:1)...\n")
...
fig_heat <- plotly::plot_ly(
  ...
  y = 1:n_tips,
  type = "heatmap",
  colorscale = "Viridis",  # 🎨 Встроенная палитра viridis
  zmin = 0, zmax = 1,
...
cat("\n📁 Созданные файлы:\n")
cat(sprintf("   • %s (стандартная визуализация)\n", basename(html_path_1)))
cat(sprintf("   • %s (кастомная визуализация с деревом)\n", basename(html_path_2)))
cat(sprintf("   • Delimitation_bGMYC_005.csv (p=0.05)\n"))
cat(sprintf("   • Delimitation_bGMYC_001.csv (p=0.01)\n"))
cat(sprintf("   • bGMYC_delimitation_results.csv\n"))
